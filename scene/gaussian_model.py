...
class GaussianModelMax(GaussianModel):
    def __init__(self, 
                 sh_degree : int):
        super().__init__(sh_degree)

    def add_densification_stats(self, viewspace_point_tensor, update_filter):
        self.xyz_gradient_accum[update_filter] = torch.maximum(self.xyz_gradient_accum[update_filter], torch.norm(viewspace_point_tensor.grad[update_filter,:2], dim=-1, keepdim=True))
        self.denom[update_filter] = 1
...
class GaussianModelFusion(GaussianModel):
    def __init__(self, sh_degree : int, source_dir : str, block_dim : list, aabb : list, scale: float, cell_iteration=30_000):
        super().__init__(sh_degree)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        # for compatibility with the LargeScene, we keep the same interface
        self.spatial_lr_scale = spatial_lr_scale
        num_cells = self.block_dim[0] * self.block_dim[1] * self.block_dim[2]

        for idx in range(int(num_cells)):
            try:
                self.update_ply(idx, self.cell_iteration)
            except:
                self.update_ply(idx, 1)
        
        print("Number of points at initialisation : ", self.get_xyz.shape[0])
        self._mask = nn.Parameter((torch.ones((self.get_xyz.shape[0], 1), device="cuda") * 5.0).requires_grad_(True))
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
        self.active_sh_degree = self.max_sh_degree
    
    def update_ply(self, cell_idx, iter):
        partial_path = os.path.join(self.source_dir+f"_cell{cell_idx}", "point_cloud_blocks", "scale_"+str(self.scale),
                                    "iteration_" + str(iter),
                                    "point_cloud.ply")
        full_path = os.path.join(self.source_dir+f"_cell{cell_idx}", "point_cloud",
                                "iteration_" + str(iter),
                                "point_cloud.ply")
        path = partial_path if os.path.exists(partial_path) else full_path
        ...
        cell_mask = np.ones((xyz.shape[0]), dtype=bool)
        if not os.path.exists(partial_path):
            cell_mask = block_filtering(cell_idx, xyz, self.aabb, self.block_dim, self.scale)

            mkdir_p(os.path.dirname(partial_path))
            _xyz = xyz[cell_mask]
            _normals = np.zeros_like(_xyz)
            _f_dc = features_dc[cell_mask].reshape((cell_mask.sum(), -1))
            _f_rest = features_extra[cell_mask].reshape((cell_mask.sum(), -1))
            _opacities = opacities[cell_mask]
            _scale = scales[cell_mask]
            _rotation = rots[cell_mask]
            ...
            elements = np.empty(_xyz.shape[0], dtype=dtype_full)
            attributes = np.concatenate((_xyz, _normals, _f_dc, _f_rest, _opacities, _scale, _rotation), axis=1)
            elements[:] = list(map(tuple, attributes))
            el = PlyElement.describe(elements, 'vertex')
            PlyData([el]).write(partial_path)

        # no grad is required for gaussian cell fusion
        self._xyz = torch.concat([self._xyz.cuda(), torch.tensor(xyz[cell_mask], dtype=torch.float, device="cuda")], dim=0)
        self._features_dc = torch.concat([self._features_dc.cuda(), torch.tensor(features_dc[cell_mask], dtype=torch.float, device="cuda").transpose(1, 2).contiguous()], dim=0)
        self._features_rest = torch.concat([self._features_rest.cuda(), torch.tensor(features_extra[cell_mask], dtype=torch.float, device="cuda").transpose(1, 2).contiguous()], dim=0)
        self._opacity = torch.concat([self._opacity.cuda(), torch.tensor(opacities[cell_mask], dtype=torch.float, device="cuda")], dim=0)
        self._scaling = torch.concat([self._scaling.cuda(), torch.tensor(scales[cell_mask], dtype=torch.float, device="cuda")], dim=0)
        self._rotation = torch.concat([self._rotation.cuda(), torch.tensor(rots[cell_mask], dtype=torch.float, device="cuda")], dim=0)

        print("Merged {} points from cell {}.".format(cell_mask.sum(), cell_idx))

    def training_setup(self, training_args):
        self.percent_dense = training_args.percent_dense

        # only train the learnable mask
        l = [{'params': [self._mask], 'lr': training_args.mask_lr, "name": "mask"}]
    ...
    def update_learning_rate(self, iteration):
        raise NotImplementedError("Learning rate scheduling is not implemented for the GaussianModelFusion.")
    
    def reset_opacity(self):
        raise NotImplementedError("Opacity reset is not implemented for the GaussianModelFusion.")

    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        raise NotImplementedError("Densification is not implemented for the GaussianModelFusion.")
...
class GaussianModelVoxV2(GaussianModel):
    def __init__(self, sh_degree : int, 
                 xyz_range=[], 
                 voxel_size=[0.01, 0.01, 0.01], 
                 mode='gmm',
    ...
    def load_ply(self, path):
        ...
        if len(self.xyz_range) == 0:
            xyz_range = np.asarray([np.min(xyz, axis=0), np.max(xyz, axis=0)]).flatten()
        ...
        if self.apply_voxelize:
            voxel_index = torch.div(torch.tensor(xyz[:, :3]).float().cuda() - self.xyz_range[None, :3], self.voxel_size[None, :], rounding_mode='floor')
            voxel_coords = voxel_index * self.voxel_size[None, :] + self.xyz_range[None, :3] + self.voxel_size[None, :] / 2

            sh_features = np.concatenate((features_dc, features_extra), axis=2).reshape(xyz.shape[0], -1)
            feat = torch.tensor(np.concatenate([xyz, scales, rots, opacities, sh_features], axis=-1)).float().cuda()

            new_coors, unq_inv = self.scatter_gs(feat, voxel_coords, mode=self.mode)
        else:
            self._xyz = nn.Parameter(torch.tensor(xyz, dtype=torch.float, device=self.device).requires_grad_(True))
            self._features_dc = nn.Parameter(torch.tensor(features_dc, dtype=torch.float, device=self.device).transpose(1, 2).contiguous().requires_grad_(True))
            self._features_rest = nn.Parameter(torch.tensor(features_extra, dtype=torch.float, device=self.device).transpose(1, 2).contiguous().requires_grad_(True))
    ...
    def scatter_gs(self, feat, coors, mode="avg", return_inv=True, min_points=0, unq_inv=None, new_coors=None):
        ...
        self._xyz = nn.Parameter(new_feat[:, :3].requires_grad_(True))
        self._scaling = nn.Parameter(new_feat[:, 3:6].requires_grad_(True))
        self._rotation = nn.Parameter(new_feat[:, 6:10].requires_grad_(True))
        self._opacity = nn.Parameter(new_feat[:, [10]].requires_grad_(True))

        vox_sh_features = new_feat[:, 11:].reshape(-1, 3, 16)
        self._features_dc = nn.Parameter(vox_sh_features[:, :, [0]].transpose(1, 2).contiguous().requires_grad_(True))
        self._features_rest = nn.Parameter(vox_sh_features[:,:, 1:].transpose(1, 2).contiguous().requires_grad_(True))
...
class GaussianModelLoD(GaussianModel):
    def __init__(self, sh_degree : int, 
        ...
        super().__init__(sh_degree)
        self.device = device
        self.xyz_range =  torch.tensor(xyz_range, device=self.device).float()
        self.voxel_size = torch.tensor(voxel_size, device=self.device).float()
        self.lod_threshold = torch.tensor(lod_threshold+[torch.inf], device=self.device).float()
    ...
    def load_ply(self, path):
        ...
        self._xyz = nn.Parameter(self.inverse_xyz_activation(torch.tensor(xyz, dtype=torch.float, device=self.device)).requires_grad_(True))
        self._features_dc = nn.Parameter(torch.tensor(features_dc[mask], dtype=torch.float, device=self.device).transpose(1, 2).contiguous().requires_grad_(True))
        self._features_rest = nn.Parameter(torch.tensor(features_extra[mask], dtype=torch.float, device=self.device).transpose(1, 2).contiguous().requires_grad_(True))
        self._opacity = nn.Parameter(torch.tensor(opacities[mask], dtype=torch.float, device=self.device).requires_grad_(True))
        self._scaling = nn.Parameter(torch.tensor(scales[mask], dtype=torch.float, device=self.device).requires_grad_(True))
        self._rotation = nn.Parameter(torch.tensor(rots[mask], dtype=torch.float, device=self.device).requires_grad_(True))
        self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device=self.device)
        ...
        if self.apply_lod:
            try: 
                ...
                self.level_lookup = torch.tensor(level_lookup, dtype=torch.long, device=self.device)
                ...
                self.level_lookup = torch.zeros((self._xyz.shape[0], num_levels), dtype=torch.long, device=self.device)
                ...
                for level in range(num_levels):
                    ...
                    lookup_pad_list.append(torch.ones((new_coors.shape[0], 2), dtype=torch.long, device=self.device) * level)
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        prune_filter = torch.cat((selected_pts_mask, torch.zeros(range_mask.sum(), device=self.device, dtype=bool)))
        self.prune_points(prune_filter)
