...
from gaussian_renderer import render, render_v4, network_gui
import sys
    ...
    while iteration <= opt.iterations:
        ...
        for dataset_index, (cam_info, gt_image) in enumerate(data_loader):    
            ...
            if (iteration - 1) == debug_from:
                pipe.debug = True
            render_pkg = render_v4(cam_info, gaussians, pipe, background)
            ...
            with torch.no_grad():
                ...
                training_report(dataset, log_writer, image_logger, iteration, Ll1, loss, l1_loss, ema_time, lr,
                                iter_start.elapsed_time(iter_end), testing_iterations, scene, render_v4, (pipe, background))
