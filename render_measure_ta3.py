...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, pitch, traj_info):
    ...
    render_path = os.path.join(model_path, name, "ours_TA3", "renders")
    gts_path = os.path.join(model_path, name, "ours_TA3", "gt")
    ...
    for t in tqdm(np.arange(0, 1, step), desc="Rendering progress"):
        ...
        # no data saving
        # torchvision.utils.save_image(render_pkg["render"], os.path.join(render_path, '{0:05d}'.format(int(t/step)) + ".png"))
        # if idx >= 50:
        #     break
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, load_vq : bool, skip_train : bool, skip_test : bool, custom_test : bool, pitch : float, traj_info : list):
    ...
    with torch.no_grad():
        ...
        if custom_test:
            ...
            render_set(dataset.model_path, filename, scene.loaded_iter, views, gaussians, pipeline, background, pitch, traj_info)
            print("Skip both train and test, render all views")
        else:
            if not skip_train:
                render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, pitch, traj_info)

            if not skip_test:
                render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, pitch, traj_info)
