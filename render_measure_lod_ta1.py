...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, pitch, heights):
    ...
    render_path = os.path.join(model_path, name, "ours_lod_TA1", "renders")
    gts_path = os.path.join(model_path, name, "ours_lod_TA1", "gt")
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, load_vq : bool, skip_train : bool, skip_test : bool, custom_test : bool, pitch : float, heights : list):
    ...
    with torch.no_grad():
        ...
        if custom_test:
            ...
            render_set(dataset.model_path, filename, scene.loaded_iter, views, [lod_gs_0, lod_gs_1, lod_gs_2], pipeline, background, pitch, heights)
            print("Skip both train and test, render all views")
        else:
            if not skip_train:
                render_set(dataset.model_path, "train", scene.loaded_iter, views, [lod_gs_0, lod_gs_1, lod_gs_2], pipeline, background, pitch, heights)
